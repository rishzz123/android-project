...
public class BankRegister extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks,
        GoogleApiClient.OnConnectionFailedListener,
        LocationListener {
    TextInputEditText ngoNameEt, ngoEmailEt, ngoPhoneEt, bankAddressEt;
    int email = 0, fromTimeZone = 0, toTimeZone = 0;
    String mFromTime, mToTime, ngoName, ngoEmail, ngoPhone, bankAddress;
    FirebaseAuth mAuth;
    String uId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bankAddressEt = findViewById(R.id.bankAddress);
        setUpGClient();
        ...
        final DatabaseReference rootRef3 = FirebaseDatabase.getInstance().getReference().child("GeoLocation").child("NGO").child(uId);
        rootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                if (snapshot.exists()){
        ...
        final DatabaseReference rootRef4 = FirebaseDatabase.getInstance().getReference().child("GeoLocation").child("Volunteer").child(uId);
        rootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                if (snapshot.exists()){
        ...
        findViewById(R.id.saveButton).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                bankAddress = bankAddressEt.getText().toString();
                ...
                if (TextUtils.isEmpty(ngoName)) {
                    ...
                    bankAddressEt.setError("Please Enter Address");
                    bankAddressEt.requestFocus();
                } else if (fromTimeZone == 0) {
                    Toast.makeText(BankRegister.this, "Please Select Available From Time", Toast.LENGTH_SHORT).show();
                } else if (toTimeZone == 0) {
                    Toast.makeText(BankRegister.this, "Please Select Available To Time", Toast.LENGTH_SHORT).show();

                } else {
                    final ProgressDialog progressDialog = new ProgressDialog(BankRegister.this);
                    progressDialog.setMessage("Please wait");
                    progressDialog.show();


                    userDatabase = FirebaseDatabase.getInstance().getReference().child("User").child("BloodBank").child(uId).child("Blood");
                    ...
                    userDetails.put("Address", bankAddress);
                    ...
                    profileDetails.put("Address", bankAddress);
                    profileDetails.put("Email", ngoEmail);
                    profileDetails.put("Category","Blood Bank");
                    userProfile.updateChildren(profileDetails);


                    geoData = FirebaseDatabase.getInstance().getReference().child("GeoLocation").child("BloodBank");
                    GeoFire geoFire = new GeoFire(geoData);
                    geoFire.setLocation(uId, new GeoLocation(Latitude, Longitude));
                    userDatabase.setValue(userDetails).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {


                            Intent intent = new Intent(BankRegister.this, BankWorkingDays.class);
                        ...
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(BankRegister.this, (CharSequence) e, Toast.LENGTH_SHORT).show();
                        }
                    });
    ...
    public void selectFromTime(View view) {
        ...
        fromTimePickerDialog = new TimePickerDialog(BankRegister.this, new TimePickerDialog.OnTimeSetListener() {
            @Override
    ...
    public void selectToTime(View view) {
        ...
        toTimePickerDialog = new TimePickerDialog(BankRegister.this, new TimePickerDialog.OnTimeSetListener() {
            @Override
            public void onTimeSet(TimePicker timePicker, int hourOfDay, int minute) {
                toTimeZone = 1;
    ...
    private void getMyLocation() {
        if (googleApiClient != null) {
            if (googleApiClient.isConnected()) {
                int permissionLocation = ContextCompat.checkSelfPermission(BankRegister.this,
                            ...
                            switch (status.getStatusCode()) {
                                case LocationSettingsStatusCodes.SUCCESS:
                                    ...
                                    int permissionLocation = ContextCompat
                                            .checkSelfPermission(BankRegister.this,
                                                    Manifest.permission.ACCESS_FINE_LOCATION);
                                    if (permissionLocation == PackageManager.PERMISSION_GRANTED) {
                                ...
                                case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                                    ...
                                    try {
                                        ...
                                        status.startResolutionForResult(BankRegister.this,
